...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{__('home.edit')}} {{__('home.roles')}}</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
            {!! Form::open(['route'=>'roles-update' ,'role'=>'form'])!!}
                    ...
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group col-md-6" style="float: left;">
                                ...
                                <input type="text" name="title"  value="{{$item->name}}" required class="form-control  {{ $errors->has('title') ? 'is-invalid' : '' }}" id="title" placeholder="Enter title">
                                @error('title')
                            ...
                            <div class="card-header" style="background: #343a40;">
                                ...
                                <div class="card-tools">
                                    <a href="javascript:void(0);">
                                        <input type="checkbox" class="checkbox{{$permission->id}}"
                                               {{in_array($permission->id, $mypermissions) ?'checked':''}}
                                               value="{{$permission->id}}" name="permission[]" onclick="myFunction">
                                    </a>
                                </div>
                                <!-- /.card-tools -->
                            </div>
                                ...
                                @foreach(\Spatie\Permission\Models\Permission::where('parent_id',$permission->id)->get() as $perm)
                                    <div>
                                        ...
                                        <input type="checkbox" name="permission[]" value="{{$perm->id}}"
                                               {{in_array($perm->id, $mypermissions) ?'checked':''}}
                                               class="checkbox{{$permission->id}}" style="float: right;margin-top: 3px;">
                                    </div>
                                @endforeach
